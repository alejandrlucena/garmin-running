...
function _rememberHiddenKeys(keys){var map=_hiddenKeyMap();(keys||[]).forEach(function(k){if(k)map[k]=true;});}
function _forgetHiddenKeys(keys){var map=_hiddenKeyMap();(keys||[]).forEach(function(k){if(k)delete map[k];});}
...
function _addEditModeToggle(act){
  ...
  btn.onclick=function(e){
    ...
    // close camera FAB menu when entering edit mode
...
function _compactarLista(lista, kmInterval) {
  ...
  return lista.map(function(d) {
    var flat = [];
    if (d.warmup) flat.push(d.warmup);
    (d.series || []).forEach(function(s) { flat.push(s); });
    if (d.cooldown) flat.push(d.cooldown);
    if (!flat.length) return d;

    var groups = [], group = [], groupDist = 0;
    flat.forEach(function(s) {
      group.push(s);
      groupDist += s.dist_km || 0;
      if (groupDist >= kmInterval - 0.01) { groups.push(group); group = []; groupDist = 0; }
    });
...
function _reRenderCompact() {
  ...
  document.getElementById('output').innerHTML = '<div id="render-target" data-ver="2">' + lista.map(renderActividad).join('') + '<div class="firma-web">by AlejandrLucena</div></div>';
  _refreshAllRestoreBars();
  // Ensure edit-mode toggle (✏️) + row handlers are re-attached after the
  // innerHTML replacement, even if the late IIFE wrapper didn't bind us.
...
function zonaCellHtml(zonas_lap){
  ...
  const activas=zonas_lap.filter(z=>z.secs>0);
...
function _hideRow(rowId,actId){
  ...
  _pushHide(actId,[rowId],el.getAttribute('data-lbl')||'Vuelta',{
    dur:parseInt(el.getAttribute('data-dur')||0),
    speed:parseFloat(el.getAttribute('data-speed')||0),
    sport:(aw?aw.getAttribute('data-sport'):null)||'RUN',
    keys:key?[key]:[]
  });
...
function _hideBtn(idx){
  ...
  return '<span class="lap-row-actions">'
        ...
        +'<button class="lap-merge-btn" '+swallowPd+' onclick="'+swallow+'_mergeIntoGroup(\''+rowId+'\',\''+aid+'\')" title="Meter en grupo anterior">↥</button>'
        +'<button class="lap-merge-next-btn" '+swallowPd+' onclick="'+swallow+'_mergeIntoNextGroup(\''+rowId+'\',\''+aid+'\')" title="Meter en grupo siguiente">↧</button>'
        +'</span>';
}
...
function _recalcAvgRows(actId){
  ...
  var mSmaxSes=vmAll.length?maxA(vmAll,'data-smax'):0,mSmaxSer=vmAct.length?maxA(vmAct,'data-smax'):0;
  var dec=_isMoto?3:1;
  var _summaryIds=[actId+'ses-time',actId+'ses-cum',actId+'ser-time',actId+'ser-cum',actId+'ses-spd',actId+'ser-spd',actId+'ses-fcm',actId+'ser-fcm',actId+'ses-fcx',actId+'ser-fcx',actId+'ses-cad',actId+'ser-cad',actId+'ses-pac',actId+'ser-pac'];
  var _els={};_summaryIds.forEach(function(id){_els[id]=document.getElementById(id);});
  function upd(id,html){var el=_els[id];if(el)el.innerHTML=html;}
  ...
  _replaceZonaTd(actId+'ses', _zonesFromRows(allR));
  ...
  if(zonesDiv){
    var sesBlock=_renderZoneBlockGlobal('Media sesión',_zonesFromRows(allR));
    var serBlock=(!_isContinua&&!_isMoto)?_renderZoneBlockGlobal('Media trabajo efectivo',_zonesFromRows(actR)):'';
    var newInner=sesBlock+(serBlock?'<div style="margin-top:16px">'+serBlock+'</div>':'');
    if(newInner) zonesDiv.innerHTML=newInner;
  }
}
...
function renderActividad(d){
  ...
  if(window.customHRZonesActive){
    seriesArr.forEach(function(s){delete s.zonas_lap;if(s._subLaps)s._subLaps.forEach(function(sl){delete sl.zonas_lap;});});
    if(warmup){delete warmup.zonas_lap;if(warmup.subLaps)warmup.subLaps.forEach(function(sl){delete sl.zonas_lap;});}
    if(cooldown){delete cooldown.zonas_lap;if(cooldown.subLaps)cooldown.subLaps.forEach(function(sl){delete sl.zonas_lap;});}
  }
  ...
  seriesArr.forEach(function(s){_distributeZones(s,zonas,fcMinAll,fcMaxAll,fcRangeAll);_fallbackZ1(s);if(s._subLaps)s._subLaps.forEach(function(sl){_distributeZones(sl,zonas,fcMinAll,fcMaxAll,fcRangeAll);_fallbackZ1(sl);});});
  _distributeZones(warmup,zonas,fcMinAll,fcMaxAll,fcRangeAll);_fallbackZ1(warmup);if(warmup&&warmup.subLaps)warmup.subLaps.forEach(function(sl){_distributeZones(sl,zonas,fcMinAll,fcMaxAll,fcRangeAll);_fallbackZ1(sl);});
  _distributeZones(cooldown,zonas,fcMinAll,fcMaxAll,fcRangeAll);_fallbackZ1(cooldown);if(cooldown&&cooldown.subLaps)cooldown.subLaps.forEach(function(sl){_distributeZones(sl,zonas,fcMinAll,fcMaxAll,fcRangeAll);_fallbackZ1(sl);});
...
var allR=[];
  ...
  var avgSpeedMaxSes=_smsArr.length?Math.max.apply(null,_smsArr.map(function(r){return r.speed_max;})):0;
  var _smrArr=activeOnlyArr.filter(function(r){return(r.speed_max||0)>=0.3;});
  var avgSpeedMaxSer=_smrArr.length?Math.max.apply(null,_smrArr.map(function(r){return r.speed_max;})):0;
  ...
  function _dataRow(s, ref, isDesc, isPill, rowClass, hideButton){
    ...
    var labelHtml=(hideButton?_hideBtn(ri):'')
      ...
      +'<span style="'+(isDesc?'color:#252830':'')+'">'+lbl+'</span>'
      +durSpan
      +'</div></div>';
    ...
    return '<tr id="'+rowId+'"'+_dAttrs+' class="'+(rowClass||'')+rowClassDesc+'">'
        ...
        +(isIndoorCycling?'':'<td><div class="metric"><div class="main" '+descStyle+'>'+kmhVal+'</div>'+dKmhM+'</div></td>')
        ...
        +(isMoto?'':(isCycling
          ...
          :(esContinua&&esCarrera?'':'<td><div class="metric"><div class="main" '+descStyle+'>'+ritmoVal+'</div>'+dRitmoM+'</div></td>')))
      ...
      return '<tr id="'+rowId+'"'+_dAttrs+' class="'+(rowClass||'')+(isDesc?' desc-row':'')+(_isFastest?' fastest-lap':'')+'">'
        ...
        +(isIndoorCycling?'':(function(){var _spd=toKmh(s.speed);var _kpill=maxKmhVal>0&&_spd===toKmh(maxKmhVal)&&(s.dist_km||0)>=_pillMinDist&&!_residualSet.has(s);var _pc='vel-med-pill';var _kdisp=_kpill?'<span class="'+_pc+'">'+_spd+' km/h</span>':_spd;return'<td><div class="metric"><div class="main">'+_kdisp+'</div>'+(hasRef?dKmh(s.speed,ref.speed):'')+'</div></td>';}()))
        ...
        +(isMoto?'':(isCycling
          ...
          :(esContinua&&esCarrera?'':(function(){var _ritmo=toRitmo(s.speed);var _rp=maxKmhVal>0&&_ritmo===toRitmo(maxKmhVal)&&(s.dist_km||0)>=_pillMinDist&&!_residualSet.has(s);return'<td><div class="metric"><div class="main">'+(_rp&&_ritmo?'<span class="ritmo-pill">'+_ritmo+'</span>':_ritmo)+'</div>'+(hasRef?dRitmo(s.speed,ref.speed):'')+'</div></td>';}()))))
  ...
  function _summaryRow(s, rowClass, label, cumSecs, zonasForRow, ref){
    ...
    var attrs=canToggle?' id="'+gid+'" onclick="_toggleGroup(\''+gid+'\')" data-lbl="'+label.replace(/"/g,'&quot;')+'" data-dur="'+dur+'" data-speed="'+(s.speed||0)+'" data-dist="'+(s.dist_km||0)+'" data-fcm="'+(s.fc_med||0)+'" data-fcx="'+(s.fc_max||0)+'" data-cad="'+(s.cadencia||0)+'" data-pow="'+(s.potencia_w||0)+'" data-smax="'+(s.speed_max||0)+'" data-dsn="'+(parseInt(s.desnivel)||0)+'"':'';
    var firstCell=(canToggle?'<span class="group-arrow">▼</span>':'')+label
    ...
    var _isFastestSum;
    if(s._isFastestGroup!==undefined){_isFastestSum=s._isFastestGroup;}
    ...
    var _isSummaryFastest= maxSpdSum>0 && s.speed>=maxSpdSum-0.001 && (s.dist_km||0)>=_pillMinDist && !_residualSet.has(s);
    return '<tr'+attrs+' class="'+rowClass+' step-summary'+(_isFastestSum?' fastest-lap':'')+'">'
      ...
      +(isIndoorCycling?'':(function(){var _spd=toKmh(s.speed);return'<td><div class="metric"><div class="main">'+(_spd?(_isSummaryFastest?'<span class="vel-med-pill">'+_spd+' km/h</span>':_spd+' km/h'):'—')+'</div>'+dKmhH+'</div></td>';}()))
      ...
      +(isMoto?'':(isCycling
        ...
        :(function(){var _ritmoS=toRitmo(s.speed);return'<td><div class="metric"><div class="main">'+(_isSummaryFastest&&_ritmoS?'<span class="ritmo-pill">'+_ritmoS+'</span>':_ritmoS)+'</div>'+dRitmoH+'</div></td>';}())))
      ...
      +zonaCellHtml(zonasForRow||s.zonas_lap)+'</tr>';
  }
      ...
      if(!_isDescanso(s)){
        ...
        while(i<laps.length){
          ...
          if(!_isDescanso(next)){
            ...
            // Residual laps (tiny auto-lap artefacts) always stick to the current group.
  ...
  if(esIntervalos&&!esContinua){
    ...
    if(isCompactedView){
      ...
      if(warmup){
        ...
        rows+=_dataRow(warmup,null,false,false,'warmup-row group-lap',true);
      ...
      if(cooldown){
        ...
        rows+=_dataRow(cooldown,lastActiveLap,false,false,'cooldown-row group-lap',true);
      }
    } else {
      ...
      if(groups.length){
        ...
        if(_lastG.type==='interval'){
          ...
          if(_prevAct&&_trailDsc.length){
            ...
            var _shouldReclaim=_residualSet.has(_prevAct)||(_avgActDur>0&&_trailDur>_avgActDur);
            if(_shouldReclaim){
  ...
  var zonasGlobal=aggregateZones(_allRNoRes);if(!zonasGlobal.length)zonasGlobal=aggregateZones(allR);if(!zonasGlobal.length)zonasGlobal=zonas;
  ...
  if(esContinua){
    ...
    rows+='<tr id="'+_actId+'ses" class="avg-row"><td>Media sesión<button class="hide-btn" onclick="event.stopPropagation();_hideSummaryRow(\''+_actId+'ses\',\''+_actId+'\')" title="Ocultar fila">✕</button></td>'
      ...
      +(isIndoorCycling?'':'<td id="'+_actId+'ser-dist" class="col-dist" style="padding-left:14px;padding-right:14px"><div class="metric"><div class="main">'+avgDSer+'</div>'+elevHtml(avgDsnivelSer)+'</div></td>')
      ...
      +zonaCellHtml(zonasSeries)+'</tr>'));
  }

  // Zona blocks
  var zHtml='';
  var blockSes=_renderZoneBlockGlobal('Media sesión',zonasGlobal);
  var blockSer=(esIntervalos&&!isMoto)?_renderZoneBlockGlobal('Media trabajo efectivo',zonasSeries):'';
  if(blockSes||blockSer){
    zHtml='<div class="zones">'+blockSes+(blockSer?'<div style="margin-top:16px">'+blockSer+'</div>':'')+'</div>';
  }
  ...
  var statsHtml='<div class="session-stats">'
    ...
    +(esIntervalos?'<div class="stat-chip"><span class="stat-lbl">Tiempo activo</span><span class="stat-val">'+fmtDur(activeSecs)+'</span></div>':'')
    +'</div>';

  var lblContent=(d.fecha||'')+' — '+(d.nombre||'')+' — '+(d.tipo||'')+' — '+(d.distancia_total||'');

  return'<div class="actividad" id="act-'+_actId+'" data-sport="'+(isMoto?'MOTO':isCycling?'BICI':'RUN')+'" data-indoor="'+(isIndoorCycling?1:0)+'" data-continua="'+(esContinua?1:0)+'">'
...
function fromChatGPTNested(raw){
  ...
  const tipo=({treadmill_running:'CINTA',running:'ASFALTO',cycling:'BICI',swimming:'NATACIÓN',
    road_running:'ASFALTO',trail_running:'TRAIL',indoor_cycling:'BICI',
    gravel_cycling:'BICI',road_biking:'BICI',mountain_biking:'BICI',
    motorcycling:'MOTO',driving:'MOTO',other_wheeled_transport:'MOTO',
  ...
  })[raw.type||raw.activity_type||'']||'CINTA';
  ...
  raw.series.forEach(sg=>{
      ...
      (sg.laps||[]).forEach(lap=>{
        // RECOVERY = descanso between intervals; COOLDOWN = enfriamiento at end
      ...
      if(isWarm&&!series.length&&!warmup){s.label='Calentamiento';warmup=s;}
      else if(isCool&&!isRec){s.label=(lbl.includes('cooldown')||lbl.includes('enfriamiento'))?'Enfriamiento':'Recuperación';cooldown=s;}
      else if(isRec){
...
function _normLap(lap){
  ...
  const raw_type=(lap.intensityType||lap.type||lap.lapType||'ACTIVE').toUpperCase();
  // Map variants
  const intensityMap={
  ...
  const intensity=intensityMap[raw_type]||'ACTIVE';
  ...
  const wktStepIndex=(lap.wktStepIndex!==undefined&&lap.wktStepIndex!==null)?lap.wktStepIndex:null;
...
function fromRawGarmin(raw){
  ...
  const tipo=_tipoMap[raw.activity_type||raw.type||'']||'CINTA';
  ...
  function buildZones(zonesArr){
    return zonesArr.filter(z=>z.secsInZone>0)
      .sort((a,b)=>b.zoneNumber-a.zoneNumber)
      .map(z=>({
  ...
  function lapZones(normLap){
    if(normLap.hrTimeInZone&&normLap.hrTimeInZone.length){
      ...
      if(dur&&tot>0&&Math.abs(tot-dur)>1){const sc=dur/tot;zones=zones.map(z=>({...z,secs:Math.round(z.secs*sc)}));}
  ...
  if(trailingStart<laps.length){
    ...
    if(firstStep!==null&&firstStep!==undefined){
      // Reliable step data: laps sharing firstStep = descanso, rest = cooldown
      ...
      // No wktStepIndex: first trailing lap = descanso, rest = cooldown
      lastRealIdx=trailingStart;
    }
  }
  ...
  if(cooldownLaps.length){
    ...
    const subLaps=cooldownLaps.length>1?cooldownLaps.map((lap)=>{
  ...
  if(warmupRawLaps.length){
    ...
    const wSpd=totDur>0?totDist/totDur:0;
    const subLaps=warmupRawLaps.length>1?warmupRawLaps.map((lap)=>{
      var cad=lap.avgRunningCadence||lap.averageRunCadence||lap.avg_running_cadence||lap.avg_cadence||lap.avg_run_cadence||lap.averageCadence||lap.cadencia||lap.cadence||0;
...
function render(){
  ...
  if(raw.includes('SortableTable_tableRow')||raw.includes('tab-splits')||raw.includes('data-title="Vueltas"')||raw.includes('data-title="Tiempo"')){
    ...
    if(gcAct){
      ...
      document.getElementById('output').innerHTML=`<div id="render-target" data-ver="2">${_listaGC.map(renderActividad).join('')}<div class="firma-web">by AlejandrLucena</div></div>`;
  ...
  document.getElementById('output').innerHTML=`<div id="render-target" data-ver="2">${listaToRender.map(renderActividad).join('')}<div class="firma-web">by AlejandrLucena</div></div>`;
  document.getElementById('btn-img').style.display='inline-flex';
  document.getElementById('btn-copy-img').style.display='inline-flex';
...
function captureScale(forDrive){
  if(isIOS())return forDrive?3:3;
  return forDrive?3.5:8;
}
...
function openIOSave(blob, btn){
  ...
  overlay.style.cssText='position:fixed;inset:0;background:rgba(0,0,0,.88);z-index:3000;'
    ...
    +'padding:16px;gap:12px;overflow-y:auto;-webkit-overflow-scrolling:touch';
  const close=()=>{URL.revokeObjectURL(url);overlay.remove();document.removeEventListener('keydown',_esc);};
  ...
  row.appendChild(dl); row.appendChild(closeBtn);
  overlay.appendChild(p); overlay.appendChild(img); overlay.appendChild(row);
...
function applyFilter(){
  ...
  if(!q)return;
  const keys=[];
  document.querySelectorAll('#render-target tbody tr').forEach(tr=>{
    const lbl=tr.querySelector('td:first-child');
    if(lbl&&lbl.textContent.toLowerCase().includes(q)){
...
function showAllRows(){
  const keys=[];
  document.querySelectorAll('#render-target tbody tr.row-hidden').forEach(tr=>{
    tr.classList.remove('row-hidden');
    const k=tr.getAttribute('data-hide-key');
    if(k)keys.push(k);
  });
  if(keys.length)_forgetHiddenKeys(keys);
}
...
function fitToGarminJson(fitData, filename) {
  ...
  var intensityMap = { warmup: 'WARMUP', active: 'ACTIVE', rest: 'REST', recovery: 'RECOVERY', cooldown: 'RECOVERY' };
...
function toggleHRZones() {
  ...
  if (window.customHRZonesActive && window._originalJson) {
...
function recomputeHRZonesFromRecords() {
  ...
  if (!window._originalJson) window._originalJson = document.getElementById('json-input').value;
  ...
  if (jsonText) {
    try {
      ...
      } else if (obj.laps && Array.isArray(obj.laps)) {
        var hasLapZones = obj.laps.some(function(l){return l.hrTimeInZone && l.hrTimeInZone.length===5;});
        if (hasLapZones) {
          var zs=[0,0,0,0,0];
          var maxObs=0;
          obj.laps.forEach(function(lap){
            var tiz=lap.hrTimeInZone;if(!tiz||tiz.length!==5)return;
            for(var zi=0;zi<5;zi++){
              var lo=(zi===0)?0:tiz[zi].zoneLowBoundary;
              var hi=(zi<4)?(tiz[zi+1].zoneLowBoundary-1):250;
              var sec=tiz[zi].secsInZone||0;if(!sec)continue;
              var oW=hi-lo+1;if(oW<=0)continue;
              for(var cj=0;cj<5;cj++){
                var cLo=cj===0?0:customZonesData[cj].min,cHi=customZonesData[cj].max;
                var ovLo=Math.max(lo,cLo),ovHi=Math.min(hi,cHi);
                if(ovLo<=ovHi)zs[cj]+=sec*((ovHi-ovLo+1)/oW);
              }
            }
            var lm=lap.maxHR||lap.fc_max||0;if(lm>maxObs)maxObs=lm;
          });
          if(maxObs>customZonesData[4].max){
            var c5ceil=maxObs+20;
            obj.laps.forEach(function(lap){
              var tiz2=lap.hrTimeInZone;if(!tiz2||tiz2.length!==5)return;
              var z5o=tiz2[4].secsInZone||0;if(!z5o)return;
              var oLo5=tiz2[4].zoneLowBoundary,oHi5=c5ceil;
              var cLo5=customZonesData[4].min,cHi5=customZonesData[4].max;
              var ovLo5=Math.max(oLo5,cLo5),ovHi5=Math.min(oHi5,cHi5);
              var oW5=oHi5-oLo5+1;
              if(oW5>0&&ovLo5<=ovHi5)zs[4]=zs[4]-z5o+z5o*((ovHi5-ovLo5+1)/oW5);
            });
          }
          if(zs.some(function(s){return s>1;})){
            existingTimes=zs.map(function(s,i){return{zoneNumber:i+1,secsInZone:Math.round(s),zoneLowBoundary:customZonesData[i].min};});
          }
        }
      }
      ...
      if (obj.laps && Array.isArray(obj.laps)) {
        obj.laps.forEach(function(lap) { delete lap.zonas_lap; delete lap.hrTimeInZone; delete lap.hr_time_in_zones; });
      }
...
function _renderConnectorActs(acts) {
  ...
  list.innerHTML = acts.map(function(a) {
    var name = a.activityName || 'Actividad';
    ...
    var type = _CONNECTOR_TIPOS[a.activityType] || (a.activityType || '');
...
(function(){
  ...
  function _aggregateRows(trs){
    ...
    trs.forEach(function(tr){
      ...
      if(sm>0&&isActive) smMax=Math.max(smMax,sm);
    });
    return {
  ...
  function _buildHeaderHtml(actId, groupId, label, agg, cumSecs, ref){
    var act = _actEl(actId); if(!act) return '';
    var table = act.querySelector('table'); if(!table) return '';
    ...
    var isMaxSpeed = !_short && agg.speed>=0.01 && maxes.speed>=0.01 && Math.abs(agg.speed-maxes.speed)<0.001;
    var isMaxVmax  = !_short && agg.smax>=0.3 && maxes.smax>=0.3 && Math.abs(agg.smax-maxes.smax)<0.001;
    var isMaxCad   = !_short && agg.cad>0 && maxes.cad>0 && Math.round(agg.cad)===Math.round(maxes.cad);
    var isMaxPow   = !_short && agg.pow>0 && maxes.pow>0 && Math.round(agg.pow)===Math.round(maxes.pow);
      ...
      switch(type){
        case 'label':
          return '<td>'
            ...
            + '</td>';
        case 'lap':      return '<td class="col-lap" style="font-weight:600"></td>';
        case 'time': {
        ...
        case 'pace': {
          var pv = agg.speed>=0.1?_toRitmo(agg.speed):'—';
          var pDisp = (isMaxRitmo && agg.speed>=0.1)?'<span class="ritmo-pill">'+pv+'</span>':pv;
    ...
    var attrs = ' data-custom-group="'+groupId+'" data-lbl="'+(label||'').replace(/"/g,'&quot;')
              ...
              + ' data-zones="'+zJson+'"';
    return '<tr id="'+groupId+'"'+attrs+' onclick="_toggleGroup(\''+groupId+'\')" class="custom-group-header step-summary">'+cells.join('')+'</tr>';
  }
  ...
  function _autoHeaderChildren(headerTr){
    ...
    while(next){
      ...
      if(next.classList.contains('group-header')||next.classList.contains('phase-header')) break;
  ...
  function _setAutoHeaderLabel(headerTr, newLabel){
    ...
    // Quitar todos los nodos de texto sueltos (la etiqueta vieja vive ahí).
  ...
  function _updateHeaderCells(headerTr, agg, cumSecs, children){
    ...
    var _isMaxSpeed = !_short && agg.speed>=0.01 && maxes.speed>=0.01 && Math.abs(agg.speed-maxes.speed)<0.001;
    var _isMaxVmax  = !_short && agg.smax>=0.3 && maxes.smax>=0.3 && Math.abs(agg.smax-maxes.smax)<0.001;
    var _isMaxCad   = !_short && agg.cad>0 && maxes.cad>0 && Math.round(agg.cad)===Math.round(maxes.cad);
    var _isMaxPow   = !_short && agg.pow>0 && maxes.pow>0 && Math.round(agg.pow)===Math.round(maxes.pow);
    ...
    types.forEach(function(t, i){
      ...
      switch(t){
        ...
        case 'pace': setMain(td, _isMaxSpeed?'<span class="ritmo-pill">'+_toRitmo(agg.speed)+'</span>':(agg.speed>=0.1?_toRitmo(agg.speed):'—'), ref?dRitmo(agg.speed,ref.speed):''); break;
  ...
  function _recalcAutoHeaders(actId){
    ...
    headers.forEach(function(h){
      ...
      _setAutoHeaderLabel(h, _deriveLabelFromChildren(children));
  ...
  function _recalcCustomGroups(actId){
    ...
    act.querySelectorAll('tr.custom-group-header').forEach(function(headerTr){
      ...
      var ref = _aggFromHeader(_prevPeerHeader(headerTr));
      var newHtml = _buildHeaderHtml(actId, gid, label, agg, cumSecs, ref);
      var tmp = document.createElement('tbody');
  ...
  function _materializeGroup(actId, groupId, label, childRows, skipPush){
    ...
    var headerHtml = _buildHeaderHtml(actId, groupId, label, agg, 0);
    var tmp = document.createElement('tbody');
  ...
  function _disbandGroup(actId, groupId){
    ...
    children.forEach(function(tr){
      ...
      // keep .group-lap only if it wasn't original; safer: remove if it has custom-indent
      if(tr.querySelector('.custom-indent')) tr.classList.remove('group-lap');
      _removeRowIndent(tr);
    });
    header.remove();
    _saveCustomGroupsState(actId);
  ...
  W._mergeIntoGroup = function(rowId, actId){
    ...
    // p es una fila dentro de un grupo auto → mover tr después de p (queda en el auto group).
  ...
  W._mergeIntoNextGroup = function(rowId, actId){
    ...
    if(!n) return;
    if(n.classList.contains('avg-row')||n.classList.contains('avg-act')) return;
    // n es un header custom → meter al inicio de ese grupo.
    if(n.classList.contains('custom-group-header')){
      _opMove(actId, rowId, n.id, n.id);
      return;
    }
  ...
  function _ensureActionsColumn(table){
    ...
    var SEL = ['.lap-row-actions','.lap-handle','.hide-btn','.lap-extract-btn','.lap-merge-btn','.lap-merge-next-btn','.ungroup-btn'];
    var trs = table.querySelectorAll(':scope > tbody > tr');
    trs.forEach(function(tr){
  ...
  function _updateFabState(){
    ...
    // Antes solo se mostraba en móvil; ahora también en escritorio porque
    // el undo/redo es igual de útil con la tabla larga.
    if(hasAct) fab.classList.add('visible'); else fab.classList.remove('visible');
  ...
  (function _observeBtnImg(){
    ...
    ['btn-img','btn-copy-img','btn-link-img'].forEach(function(id){
